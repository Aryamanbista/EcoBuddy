...
const NotificationIcon = ({ type }) => {
  const icons = {
    ...
    success: <FaCalendarCheck className="text-green-500" />,
    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow-md max-w-4xl mx-auto">
        <div className="p-4 border-b flex justify-end">
            <button className="text-sm text-green-600 font-semibold hover:underline">Mark all as read</button>
        </div>
        <ul className="divide-y divide-gray-200">
          {mockNotifications.map(notif => (
            <li key={notif.id} className={`p-4 flex items-start space-x-4 ${!notif.read ? 'bg-green-50' : 'bg-white'}`}>
              <NotificationIcon type={notif.type} />
              <div className="flex-1">
                <div className="flex justify-between items-center">
                    <h4 className="font-bold text-gray-800">{notif.title}</h4>
                    {!notif.read && <span className="w-2.5 h-2.5 bg-green-500 rounded-full"></span>}
                </div>
